...
class SeqUnitStacked(object):
    ...
    def build_encoder(self):
        ...
        with tf.variable_scope('encoder'):
            ...
            self.f_encoder_cell = self.build_encoder_cell()
            self.b_encoder_cell = self.build_encoder_cell()
            ...
            with tf.device('/gpu:0'):
                (self.encoder_outputs, output_state_fw, output_state_bw) = stack_bidirectional_dynamic_rnn(
                    cells_fw=self.f_encoder_cell, cells_bw=self.b_encoder_cell, inputs=self.encoder_inputs_embedded,
                    sequence_length=self.encoder_inputs_length,
                    dtype=self.dtype)

                print("outputs ", self.encoder_outputs)
                print("states forward ", output_state_fw)
                print("states backward ", output_state_bw)
                ...
                if (self.cell_type.lower() == 'lstm'):
                    ...
                    #Copy the final state of the encoder as the initial state of all decoder layers
                    for x in range(0, self.depth):
                        self.encoder_last_state=self.encoder_last_state+(tf.contrib.rnn.LSTMStateTuple(c=encoder_final_state_c, h=encoder_final_state_h),)
                if (self.cell_type.lower() == 'gru'):
                    #Copy the final state of the encoder as the initial state of all decoder layers
                    for x in range(0, self.depth):
                        self.encoder_last_state=self.encoder_last_state+ (tf.concat((output_state_fw[-1], output_state_bw[-1]), axis=1),)

                print("Encoder final encoder_output ", self.encoder_outputs)
                print("Encoder final encoder_laststate ", self.encoder_last_state)

    # Building encoder cell
    def build_encoder_cell (self):
        with tf.device('/gpu:0'):
            return [self.build_single_cell() for i in range(self.depth)]
    ...
    def build_decoder_cell(self):
        ...
        with tf.device('/gpu:0'):
            self.attention_mechanism = attention_wrapper.BahdanauAttention(
                num_units=self.hidden_units*2, memory=encoder_outputs,
        ...
        self.decoder_cell_list = [
            self.build_single_cell2() for i in range(self.depth)]
        ...
        def attn_decoder_input_fn(inputs, attention):
            ...
            _input_layer = Dense(self.hidden_units*2, dtype=self.dtype,
    ...
    def build_single_cell2(self):
        cell_type = LSTMCell
        if (self.cell_type.lower() == 'gru'):
            cell_type = GRUCell

        cell = cell_type(self.hidden_units*2)
        #print(cell)
        if self.use_dropout:
            cell = DropoutWrapper(cell, dtype=self.dtype,
                                  output_keep_prob=self.keep_prob_placeholder, )
